...
export default class HomeComponent extends Component {
    ...
    componentDidMount(){
        ...
        if(registerSuccess === true || registerSuccess === "true"){
            toast('You have successfully registered. To access your account, please login now');
            setTimeout(function(){
        ...
        if(tableId !== undefined && tableId !== null && tableId !== ''){
            axiosInstance.get(`api/restaurant/table/`+tableId)
            .then(res => {
                //console.log('res',res)
                var restaurantRes = res.data.data;
                var restaurantInfo = restaurantRes.restaurant !== undefined && restaurantRes.restaurant !== null && restaurantRes.restaurant !== '' ? restaurantRes.restaurant : '';
                var tableInfo = restaurantRes.table !== undefined && restaurantRes.table !== null && restaurantRes.table !== '' ? restaurantRes.table : '';
                ...
                if(restaurantInfo !== ''){
                    restaurant_menu_pdf = restaurantInfo.restaurant_menu_pdf !== undefined && restaurantInfo.restaurant_menu_pdf !== null && restaurantInfo.restaurant_menu_pdf !== '' ? restaurantInfo.restaurant_menu_pdf : '';
                    restaurant_name = restaurantInfo.restaurant_name !== undefined && restaurantInfo.restaurant_name !== null && restaurantInfo.restaurant_name !== '' ? restaurantInfo.restaurant_name : '';
                    restaurant_photos = restaurantInfo.restaurant_photos !== undefined && restaurantInfo.restaurant_photos !== null && restaurantInfo.restaurant_photos !== '' ? restaurantInfo.restaurant_photos : [];
                }

                if(tableInfo !== ''){
                    table_name = tableInfo.name !== undefined && tableInfo.name !== null && tableInfo.name !== '' ? tableInfo.name : '';
                    seat = tableInfo.seat !== undefined && tableInfo.seat !== null && tableInfo.seat !== '' ? tableInfo.seat : [];
                }
                ...
                var restaurantInfo = {
                    restaurant_menu_pdf : baseURL+'uploads/restaurant/'+restaurant_menu_pdf,
    ...
    render() {
        let { contentType,restaurantInfo } = this.state;
            ...
            <>
                ...
                {(this.state.homeCoupleBtn === true) ?
                    <figure className={'foodPrintedMenu d-block m-0 pt-4 px-3 pb-4'}>
                        {this.state.restaurantInfo.restaurant_menu_pdf !=='' &&
                            <PDF file={this.state.restaurantInfo.restaurant_menu_pdf} scale={1.2} />
                        }

                        {this.state.restaurantInfo.restaurant_menu_pdf ===''  && this.state.position === 1 &&
